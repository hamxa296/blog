...
interface CmsRouteProps {
  ...
  allowedRoles?: ('admin' | 'editor' | 'moderator' | 'author' | 'user')[];
}

export const CmsRoute: React.FC<CmsRouteProps> = ({ 
  children, 
  allowedRoles = ['admin', 'editor', 'moderator', 'author'] 
}) => {
    ...
    return (
      <div className="min-h-[calc(100vh-88px)] flex items-center justify-center bg-transparent text-white">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-[#B3CFE5] font-semibold tracking-wider animate-pulse">Checking Permissions...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="container mx-auto px-6 py-20 text-center text-white min-h-[calc(100vh-88px)] flex flex-col justify-center items-center">
        <div className="max-w-md w-full bg-black/40 p-8 rounded-3xl border border-red-500/30 backdrop-blur-md shadow-lg shadow-red-950/20">
          ...
          <p className="text-white/70 mb-6 text-sm">
            You do not have the required permissions to access this CMS section. Your current role is <strong className="uppercase">{role}</strong>.
          </p>
          <a
            href="/"
