...
function arcY(i: number, total: number, px: number): number {
  ...
  return t * t * (px * 0.22); // parabola: 0 at centre, ~9px at edges
}
...
export function ImagesBadge({
  ...
  // Mobile stays expanded; larger screens expand on hover.
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      whileHover={reduced || isMobile ? undefined : { y: -2, scale: 1.015 }}
      transition={SPRING}
    ...
    >
      ...
      <motion.div
        ...
        animate={{
          width: reduced || isMobile ? spreadW : expanded ? spreadW : collapsedW,
        }}
        transition={SPRING}
      >
        {slots.map((slot, i) => {
          ...
          const tx = expanded ? spreadX(i) : collapsedX(i);
          const ty = expanded ? arcY(i, total, px) : 0;
          const rotate = reduced
            ? 0
            : expanded
              ? (HOVER_ROT[i] ?? 0)
              : (REST_ROT[i] ?? 0);
          const opacity = isHidden ? (expanded ? 1 : 0) : 1;
          const scale = isHidden ? (expanded ? 1 : 0.6) : 1;
          // Front-most card has highest z when collapsed; all equal when spread
          const zIdx = expanded ? i + 1 : total - i;
          ...
          return (
            <motion.div
              ...
              animate={{
                x: reduced || isMobile ? spreadX(i) : tx,
                y: reduced || isMobile ? (isMobile ? arcY(i, total, px) : 0) : ty,
                rotate: rotate,
                opacity: isMobile && isHidden ? 1 : opacity,
                scale: isMobile && isHidden ? 1 : scale,
              }}
              transition={{
                ...SPRING,
                delay:
                  !reduced && !isMobile && isHidden
                    ? (i - maxVisible) * 0.06
                    : 0,
              }}
            >
              {isOverflow ? (
                <span
                  className={cn("font-semibold text-muted-foreground", cnt)}
                >
                  +{overflow}
                </span>
              ) : (
