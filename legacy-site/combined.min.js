 ...
 * Build Date: 2025-08-21T01:19:30.881Z
 */

// Safety check: Prevent blog post related errors
if (typeof window.displayRecentPosts === 'undefined') {
    window.displayRecentPosts = function() {
        console.log('Blog post functionality has been removed');
        return;
    };
}
if (typeof window.displayApprovedPosts === 'undefined') {
    window.displayApprovedPosts = function() {
        console.log('Blog post functionality has been removed');
        return;
    };
}
if (typeof window.displayFeaturedPost === 'undefined') {
    window.displayFeaturedPost = function() {
        console.log('Blog post functionality has been removed');
        return;
    };
}

// Safety check: Prevent DOM element errors
if (typeof window.recentPostsSkeleton === 'undefined') {
    window.recentPostsSkeleton = null;
}
...
// The firebaseConfig should be loaded from firebase-config.js before this file
if (typeof window.firebaseConfig === 'undefined') {
    console.warn('Firebase config not found. Please ensure firebase-config.js is loaded before this file.');
    // Fallback to placeholder values (will not work in production)
    window.firebaseConfig = {
        apiKey: "CONFIG_NOT_LOADED",
...
// Use the config from window without redeclaring
...
let hasMorePhotos = false; // Start with false, will be set to true only if there are more photos
let isLoading = false;
...
async function loadPhotosByStatus(status, page = 0, append = false) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Only show "Load More" if we got a full page of photos
            // If we get fewer photos than the limit, we've reached the end
            hasMorePhotos = result.photos.length === PHOTOS_PER_PAGE;
            currentPage = page;
            // Also update global variables for the HTML gallery functions
            window.hasMorePhotos = result.photos.length === PHOTOS_PER_PAGE;
            window.currentPage = page;
            window.currentPhotoStatus = status;
        } else {
...
async function loadMorePhotos() {
    ...
    if ((!hasMorePhotos && !window.hasMorePhotos) || isLoading) return;
    
    const nextPage = (currentPage || window.currentPage || 0) + 1;
    const status = currentPhotoStatus || window.currentPhotoStatus || 'approved';
    await loadPhotosByStatus(status, nextPage, true);
}
...
function renderPhotoGrid(photos, status) {
    ...
    // Also remove any existing load more button that might be outside the grid
    ...
    if (photos.length === 0) {
        ...
        noPhotosDiv.className = 'col-span-full text-center py-12 text-gray-500';
        ...
        path.setAttribute('d', 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 0 002 2z');
        
        svg.appendChild(path);
        
        const h3 = document.createElement('h3');
        h3.className = 'text-xl font-semibold mb-2';
        h3.textContent = `No ${status} photos`;
        
        const p = document.createElement('p');
        p.textContent = `There are no ${status} photos to display.`;
        
        noPhotosDiv.appendChild(svg);
        noPhotosDiv.appendChild(h3);
        noPhotosDiv.appendChild(p);
    ...
    // Check both local and global variables
    if (hasMorePhotos || window.hasMorePhotos) {
        addLoadMoreButton();
    }
...
function addLoadMoreButton() {
    ...
    // Double-check that we actually have more photos to load
    // Check both local and global variables
    if (!hasMorePhotos && !window.hasMorePhotos) {
        return;
    }
    ...
    button.className = 'icy-btn px-8 py-3 text-lg font-semibold';
...
function createPhotoCard(photo, status) {
    ...
    // Create card content safely without innerHTML
    ...
    img.setAttribute('src', 'data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 400 300\'%3E%3Crect width=\'400\' height=\'300\' fill=\'%23f3f4f6\'/%3E%3C/svg%3E');
    img.setAttribute('data-src', optimizedImageUrl);
    img.setAttribute('alt', photo.caption);
    
    // Add click handler safely
    if (isAdmin) {
        img.addEventListener('click', () => openPhotoReview(photo.id));
    } else {
        img.addEventListener('click', () => viewPhotoDetails(photo.id));
    }
    
    relativeDiv.appendChild(img);
    
    // Add status badge if admin
    if (statusBadge) {
        const badgeDiv = document.createElement('div');
        // Parse the status badge HTML safely
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = statusBadge;
        while (tempDiv.firstChild) {
            badgeDiv.appendChild(tempDiv.firstChild);
        }
        relativeDiv.appendChild(badgeDiv);
    }
    
    // Add admin buttons if admin
    if (adminButtons) {
        const adminDiv = document.createElement('div');
        // Parse the admin buttons HTML safely
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = adminButtons;
        while (tempDiv.firstChild) {
            adminDiv.appendChild(tempDiv.firstChild);
        }
        relativeDiv.appendChild(adminDiv);
    }
    ...
    metaDiv.className = 'flex items-center justify-between mb-2';
    
    const metaP = document.createElement('p');
    metaP.className = 'text-sm text-gray-500';
    metaP.textContent = `${photoDate} • by ${photo.uploaderName}`;
    
    const categorySpan = document.createElement('span');
    categorySpan.className = 'inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full';
    categorySpan.textContent = photo.category;
    ...
    h4.textContent = photo.caption;
    
    contentDiv.appendChild(metaDiv);
    contentDiv.appendChild(h4);
    
    // Add rejection reason if admin and exists
    if (isAdmin && photo.rejectionReason) {
        const rejectionP = document.createElement('p');
        rejectionP.className = 'text-sm text-red-600 mb-2';
        
        const strong = document.createElement('strong');
        strong.textContent = 'Rejection Reason: ';
        rejectionP.appendChild(strong);
        
        const reasonSpan = document.createElement('span');
        reasonSpan.textContent = photo.rejectionReason;
        rejectionP.appendChild(reasonSpan);
        
        contentDiv.appendChild(rejectionP);
    }
    
    const detailsButton = document.createElement('button');
    detailsButton.className = 'font-semibold text-blue-600 hover:underline';
    detailsButton.textContent = 'View Details →';
    
    // Add click handler safely
    if (isAdmin) {
        detailsButton.addEventListener('click', () => openPhotoReview(photo.id));
    } else {
        detailsButton.addEventListener('click', () => viewPhotoDetails(photo.id));
    }
...
async function openPhotoReview(photoId) {
    ...
    try {
        ...
        const img = document.createElement('img');
        img.src = currentPhoto.fullSizeUrl;
        img.alt = currentPhoto.caption;
        ...
        h3.textContent = currentPhoto.caption;
        
        const p = document.createElement('p');
        p.className = 'text-gray-600';
        p.textContent = currentPhoto.category;
        
        headerDiv.appendChild(h3);
        headerDiv.appendChild(p);
        
        const gridDiv = document.createElement('div');
        gridDiv.className = 'grid grid-cols-2 gap-4 text-sm';
        
        const uploaderDiv = document.createElement('div');
        const uploaderStrong = document.createElement('strong');
        uploaderStrong.textContent = 'Uploaded by: ';
        uploaderDiv.appendChild(uploaderStrong);
        uploaderDiv.appendChild(document.createTextNode(currentPhoto.uploaderName));
        
        const dateDiv = document.createElement('div');
        const dateText = currentPhoto.createdAt ? currentPhoto.createdAt.toDate().toLocaleDateString() : 'N/A';
        ...
        statusSpan.className = 'capitalize';
        statusSpan.textContent = currentPhoto.status;
        statusDiv.appendChild(statusSpan);
        
        gridDiv.appendChild(uploaderDiv);
        gridDiv.appendChild(dateDiv);
        gridDiv.appendChild(statusDiv);
        
        // Add rejection reason if exists
        if (currentPhoto.rejectionReason) {
            const rejectionDiv = document.createElement('div');
            rejectionDiv.className = 'col-span-2';
            const rejectionStrong = document.createElement('strong');
            rejectionStrong.textContent = 'Rejection Reason: ';
            rejectionDiv.appendChild(rejectionStrong);
            rejectionDiv.appendChild(document.createTextNode(currentPhoto.rejectionReason));
            gridDiv.appendChild(rejectionDiv);
        }
        
        const checkboxDiv = document.createElement('div');
        checkboxDiv.className = 'flex items-center space-x-4';
        ...
        checkbox.id = 'highlight-photo';
        checkbox.className = 'mr-2';
        if (currentPhoto.isHighlighted) {
            checkbox.checked = true;
        }
        
        const span = document.createElement('span');
        span.textContent = 'Highlight in slideshow';
...
async function viewPhotoDetails(photoId) {
    try {
        ...
        modalContent.className = 'bg-white rounded-lg p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto';
        ...
        const h3 = document.createElement('h3');
        h3.className = 'font-semibold text-lg';
        h3.textContent = photo.caption;
        
        const categoryP = document.createElement('p');
        categoryP.className = 'text-gray-600';
        categoryP.textContent = photo.category;
        
        captionDiv.appendChild(h3);
        captionDiv.appendChild(categoryP);
        ...
        uploaderStrong.textContent = 'Uploaded by: ';
        uploaderDiv.appendChild(uploaderStrong);
        uploaderDiv.appendChild(document.createTextNode(photo.uploaderName));
        
        const dateDiv = document.createElement('div');
        const dateText = photo.createdAt ? photo.createdAt.toDate().toLocaleDateString() : 'N/A';
        const dateStrong = document.createElement('strong');
        dateStrong.textContent = 'Date: ';
        dateDiv.appendChild(dateStrong);
        dateDiv.appendChild(document.createTextNode(dateText));
        ...
        detailsDiv.appendChild(captionDiv);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (result.success) {
                ...
                const redirectTo = next ? decodeURIComponent(next) : 'index.html';
    ...
    if (postForm) {
        ...
        if (previewButton) {
            previewButton.addEventListener('click', () => {
                ...
                // Open preview in new window safely
                const previewWindow = window.open('', '_blank', 'width=800,height=600,scrollbars=yes,resizable=yes');
                if (previewWindow) {
                    // Validate previewHTML content before writing
                    const sanitizedHTML = previewHTML.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');
    ...
    if (profileForm) {
        ...
        const loadUserPosts = async (user) => {
            ...
            while (myPostsContainer.firstChild) {
                myPostsContainer.removeChild(myPostsContainer.firstChild);
            }
            if (result.success && result.posts.length > 0) {
                result.posts.forEach(post => {
                    ...
                    titleLink.className = 'font-bold text-lg hover:text-blue-600';
                    titleLink.textContent = post.title;
                    
                    const statusP = document.createElement('p');
                    statusP.className = 'text-sm text-gray-500 mt-1';
                    ...
                    const statusSpan = document.createElement('span');
                    statusSpan.className = `px-2 py-1 rounded-full text-xs font-semibold ${statusColor}`;
                    statusSpan.textContent = post.status;
                    
                    statusP.appendChild(statusSpan);
                    leftDiv.appendChild(titleLink);
                    leftDiv.appendChild(statusP);
                    
                    const editLink = document.createElement('a');
                    editLink.href = `launching-soon.html?edit=${post.id}`;
                    editLink.className = 'text-sm font-semibold text-blue-600 hover:underline';
                    editLink.textContent = 'Edit';
                    
                    postElement.appendChild(leftDiv);
                    postElement.appendChild(editLink);
                    myPostsContainer.appendChild(postElement);
                });
            } else {
                const noPostsP = document.createElement('p');
                noPostsP.className = 'text-gray-500';
                noPostsP.textContent = 'You have not written any posts yet.';
                myPostsContainer.appendChild(noPostsP);
            }
        };
    ...
    // --- Homepage Logic ---
    // Blog post functionality removed - no longer needed
    // const recentPostsGrid = document.getElementById('recent-posts-grid');
    // if (recentPostsGrid) {
    //     // Blog post display functions removed
    // }
    ...
    if (galleryGrid) {
        ...
        const renderGallery = (photos) => {
            // Clear gallery grid safely
            while (galleryGrid.firstChild) {
                galleryGrid.removeChild(galleryGrid.firstChild);
            }
            
            if (photos.length === 0) {
                const noPhotosP = document.createElement('p');
                noPhotosP.className = 'text-gray-500';
                noPhotosP.textContent = 'No photos found for this category.';
                galleryGrid.appendChild(noPhotosP);
                return;
            }
            photos.forEach(photo => {
                ...
                // Create photo card content safely without innerHTML
                const imageDiv = document.createElement('div');
                imageDiv.className = 'relative flex items-center justify-center';
                ...
                img.setAttribute('loading', 'lazy');
                img.setAttribute('data-fullsize', photo.fullSizeUrl || photo.imageUrl);
                ...
                captionP.className = 'font-semibold mb-2';
                captionP.textContent = photo.caption;
                
                const categoryP = document.createElement('p');
                categoryP.className = 'text-sm';
                categoryP.textContent = photo.category;
                
                const uploaderP = document.createElement('p');
                uploaderP.className = 'text-xs';
                uploaderP.textContent = `By ${photo.uploaderName}`;
                
                contentDiv.appendChild(captionP);
                contentDiv.appendChild(categoryP);
                contentDiv.appendChild(uploaderP);
                
                photoCard.appendChild(imageDiv);
                photoCard.appendChild(contentDiv);
        ...
        const loadHighlightedSlideshow = async () => {
            ...
            if (result.success && result.photos.length > 0) {
                ...
                // Function to show current slide safely without innerHTML
                const showSlide = (index) => {
                    const photo = highlightedPhotos[index];
                    
                    // Clear section safely
                    while (highlightedPhotoSection.firstChild) {
                        highlightedPhotoSection.removeChild(highlightedPhotoSection.firstChild);
                    }
                    ...
                    clickableDiv.className = 'cursor-pointer transform hover:scale-105 transition-transform duration-200';
                    clickableDiv.addEventListener('click', () => openFullscreenView(photo));
                    
                    const imageContainer = document.createElement('div');
                    imageContainer.className = 'flex items-center justify-center rounded-xl overflow-hidden';
                    ...
                    clickableDiv.appendChild(imageContainer);
                    
                    // Add dots if multiple photos
                    if (highlightedPhotos.length > 1) {
                        const dotsDiv = document.createElement('div');
                        dotsDiv.className = 'absolute top-4 right-4 flex space-x-2';
                        
                        highlightedPhotos.forEach((_, i) => {
                            const dot = document.createElement('div');
                            dot.className = `w-3 h-3 rounded-full ${i === index ? 'bg-white' : 'bg-white bg-opacity-50'}`;
                            dotsDiv.appendChild(dot);
                        });
                        
                        clickableDiv.appendChild(dotsDiv);
                    }
                    
                    const textDiv = document.createElement('div');
                    textDiv.className = 'mt-4 text-center';
                    ...
                    const categoryP = document.createElement('p');
                    categoryP.className = 'text-lg';
                    categoryP.textContent = photo.category;
                    
                    const uploaderP = document.createElement('p');
                    uploaderP.className = 'text-sm mt-1';
                    uploaderP.textContent = `By ${photo.uploaderName}`;
                    
                    textDiv.appendChild(h3);
                    textDiv.appendChild(categoryP);
                    textDiv.appendChild(uploaderP);
                    
                    mainDiv.appendChild(clickableDiv);
                    mainDiv.appendChild(textDiv);
                    
                    highlightedPhotoSection.appendChild(mainDiv);
                };
                ...
                if (allPhotos.length > 0) {
                    ...
                    const clickableDiv = document.createElement('div');
                    clickableDiv.className = 'cursor-pointer transform hover:scale-105 transition-transform duration-200';
                    clickableDiv.addEventListener('click', () => openFullscreenView(allPhotos[0]));
                    ...
                    const img = document.createElement('img');
                    img.src = allPhotos[0].imageUrl;
                    img.alt = allPhotos[0].caption;
                    img.className = 'w-full h-full object-contain rounded-xl';
                    ...
                    const h3 = document.createElement('h3');
                    h3.className = 'text-2xl font-bold mb-2';
                    h3.textContent = allPhotos[0].caption;
                    
                    const categoryP = document.createElement('p');
                    categoryP.className = 'text-lg';
                    categoryP.textContent = allPhotos[0].category;
                    
                    const uploaderP = document.createElement('p');
                    uploaderP.className = 'text-sm text-gray-500 mt-1';
                    uploaderP.textContent = `By ${allPhotos[0].uploaderName}`;
    ...
    window.openFullscreenView = function (photo) {
        ...
        // Create fullscreen modal content safely without innerHTML
        const modalContent = document.createElement('div');
        modalContent.className = 'relative w-full h-full flex items-center justify-center pointer-events-none';
        
        const closeButton = document.createElement('button');
        closeButton.className = 'absolute top-4 right-4 text-white bg-black bg-opacity-60 hover:bg-opacity-80 rounded-full w-12 h-12 flex items-center justify-center transition-all duration-200 transform hover:scale-110 shadow-lg backdrop-blur-sm z-10 pointer-events-auto';
        closeButton.addEventListener('click', () => fullscreenModal.remove());
        ...
        img.style.maxHeight = 'calc(100vh - 2rem)';
    ...
    if (adminContent) {
        ...
        const displayPosts = async () => {
            ...
            if (postsContainer) {
                ...
                if (result.success && result.posts.length > 0) {
                    ...
                    const noPostsP = document.createElement('p');
                    noPostsP.className = 'text-gray-500';
                    noPostsP.textContent = `No ${statusText} found.`;
                    postsContainer.appendChild(noPostsP);
                }
            }
        };
        ...
        function viewPost(post) {
            ...
            if (modal && modalContent) {
                ...
                titleH1.className = 'text-3xl font-bold mb-4';
                titleH1.textContent = post.title;
                
                const metaDiv = document.createElement('div');
                metaDiv.className = 'text-sm text-gray-500 mb-4';
                const postDate = post.createdAt ? post.createdAt.toDate().toLocaleDateString() : 'N/A';
                metaDiv.textContent = `By ${post.authorName} • ${postDate}`;
                
                const contentDiv = document.createElement('div');
                contentDiv.className = 'prose max-w-none';
                contentDiv.innerHTML = post.content; // This is safe as it's from the database
                
                modalContent.appendChild(titleH1);
                modalContent.appendChild(metaDiv);
                modalContent.appendChild(contentDiv);
        ...
        const checkAdminAndLoad = async () => {
            ...
            if (isAdmin) {
                ...
                while (loadingMessage.firstChild) {
                    loadingMessage.removeChild(loadingMessage.firstChild);
                }
                const accessDeniedP = document.createElement('p');
                accessDeniedP.className = 'text-lg text-red-500';
                accessDeniedP.textContent = 'Access Denied. You must be an administrator to view this page.';
                loadingMessage.appendChild(accessDeniedP);
            }
        };
    ...
    const displayApprovedPostsAdmin = async () => {
        ...
        // Clear container safely
        while (approvedPostsContainer.firstChild) {
            approvedPostsContainer.removeChild(approvedPostsContainer.firstChild);
        }
        ...
        if (result.success && result.posts.length > 0) {
            result.posts.forEach(post => {
                ...
                titleP.className = 'font-bold text-lg';
                titleP.textContent = post.title;
                const authorP = document.createElement('p');
                authorP.className = 'text-sm text-gray-500';
                authorP.textContent = `By: ${post.authorName}`;
                leftDiv.appendChild(titleP);
                leftDiv.appendChild(authorP);
                ...
                featureButton.className = `feature-btn ${featureButtonClass} text-white px-3 py-1 rounded-full text-sm font-semibold`;
                featureButton.setAttribute('data-id', post.id);
                featureButton.setAttribute('data-featured', post.isFeatured);
                featureButton.textContent = featureButtonText;
                rightDiv.appendChild(featureButton);
            ...
            const noPostsP = document.createElement('p');
            noPostsP.className = 'text-gray-500';
            noPostsP.textContent = 'There are no approved posts to feature.';
            approvedPostsContainer.appendChild(noPostsP);
        }
    ...
    const displayPendingEvents = async () => {
        ...
        try {
            ...
            if (result.success && result.events && result.events.length > 0) {
                // Clear container safely
                while (pendingEventsContainer.firstChild) {
                    pendingEventsContainer.removeChild(pendingEventsContainer.firstChild);
                }
                ...
                result.events.forEach(event => {
                    ...
                    const mainDiv = document.createElement('div');
                    mainDiv.className = 'flex justify-between items-start';
                    
                    const leftDiv = document.createElement('div');
                    leftDiv.className = 'flex-1';
                    
                    const titleH3 = document.createElement('h3');
                    titleH3.className = 'font-bold text-lg text-gray-800';
                    titleH3.textContent = event.name;
                    ...
                    locationSpan.textContent = `📍 ${event.location || 'No location'}`;
                    ...
                    submitterP.className = 'text-xs text-gray-400 mt-2';
                    submitterP.textContent = `Submitted by: ${event.submittedBy || 'Unknown'}`;
                    
                    leftDiv.appendChild(titleH3);
                    leftDiv.appendChild(descP);
                    leftDiv.appendChild(metaDiv);
                    leftDiv.appendChild(submitterP);
                    
                    const rightDiv = document.createElement('div');
                    rightDiv.className = 'flex space-x-2 ml-4';
                    
                    const approveBtn = document.createElement('button');
                    approveBtn.className = 'bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm font-medium transition-colors';
                    approveBtn.textContent = 'Approve';
                    approveBtn.addEventListener('click', () => approveEvent(event.id));
                    
                    const rejectBtn = document.createElement('button');
                    rejectBtn.className = 'bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm font-medium transition-colors';
                    rejectBtn.textContent = 'Reject';
                    rejectBtn.addEventListener('click', () => rejectEvent(event.id));
                    
                    rightDiv.appendChild(approveBtn);
                    rightDiv.appendChild(rejectBtn);
                    
                    mainDiv.appendChild(leftDiv);
                    mainDiv.appendChild(rightDiv);
                    eventElement.appendChild(mainDiv);
                    
                    pendingEventsContainer.appendChild(eventElement);
                });
            } else {
                const noEventsP = document.createElement('p');
                noEventsP.className = 'text-gray-500';
                noEventsP.textContent = 'No pending events to review.';
                pendingEventsContainer.appendChild(noEventsP);
            }
        } catch (error) {
            console.error('Error loading pending events:', error);
            const errorP = document.createElement('p');
            errorP.className = 'text-red-500';
            errorP.textContent = 'Error loading pending events.';
            pendingEventsContainer.appendChild(errorP);
        }
    };
    ...
    const displayPendingGalleryPhotos = async () => {
        ...
        try {
            ...
            if (result.success && result.photos && result.photos.length > 0) {
                // Clear container safely
                while (pendingGalleryContainer.firstChild) {
                    pendingGalleryContainer.removeChild(pendingGalleryContainer.firstChild);
                }
                ...
                result.photos.forEach(photo => {
                    ...
                    const mainDiv = document.createElement('div');
                    mainDiv.className = 'flex items-start space-x-4';
                    ...
                    const titleH3 = document.createElement('h3');
                    titleH3.className = 'font-bold text-lg text-gray-800';
                    titleH3.textContent = photo.caption;
                    
                    const categoryP = document.createElement('p');
                    categoryP.className = 'text-sm text-gray-600 mt-1';
                    categoryP.textContent = `Category: ${photo.category}`;
                    
                    const submitterP = document.createElement('p');
                    submitterP.className = 'text-xs text-gray-400 mt-2';
                    submitterP.textContent = `Submitted by: ${photo.uploaderName || 'Unknown'}`;
                    
                    const uploadDateP = document.createElement('p');
                    uploadDateP.className = 'text-xs text-gray-400';
                    const uploadDate = photo.createdAt ? new Date(photo.createdAt.toDate ? photo.createdAt.toDate() : photo.createdAt).toLocaleDateString() : 'Unknown';
                    uploadDateP.textContent = `Uploaded: ${uploadDate}`;
                    
                    contentDiv.appendChild(titleH3);
                    contentDiv.appendChild(categoryP);
                    contentDiv.appendChild(submitterP);
                    contentDiv.appendChild(uploadDateP);
                    ...
                    approveBtn.className = 'bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm font-medium transition-colors';
                    approveBtn.textContent = 'Approve';
                    approveBtn.addEventListener('click', () => approveGalleryPhoto(photo.id));
                    
                    const rejectBtn = document.createElement('button');
                    rejectBtn.className = 'bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm font-medium transition-colors';
                    rejectBtn.textContent = 'Reject';
                    rejectBtn.addEventListener('click', () => rejectGalleryPhoto(photo.id));
                    ...
                    photoElement.appendChild(mainDiv);
                ...
                const noPhotosP = document.createElement('p');
                noPhotosP.className = 'text-gray-500';
                noPhotosP.textContent = 'No pending gallery photos to review.';
                pendingGalleryContainer.appendChild(noPhotosP);
            }
        } catch (error) {
            console.error('Error loading pending gallery photos:', error);
            const errorP = document.createElement('p');
            errorP.className = 'text-red-500';
            errorP.textContent = 'Error loading pending gallery photos.';
            pendingGalleryContainer.appendChild(errorP);
        }
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createEventDetailsCard(event) {
        ...
        dateP.className = 'popup-date';
        dateP.textContent = formatEventDateTime(event.date, event.time);
        
        const tagsDiv = document.createElement('div');
        tagsDiv.className = 'popup-tags';
        const tagSpan = document.createElement('span');
        tagSpan.className = `event-tag tag-${event.type.toLowerCase()}`;
        tagSpan.textContent = event.type;
        tagsDiv.appendChild(tagSpan);
        
        const descP = document.createElement('p');
        descP.className = 'popup-description';
        descP.textContent = event.description;
        
        const locationDiv = document.createElement('div');
        locationDiv.className = 'popup-location-container';
        ...
        locationSvg.setAttribute('class', 'bi bi-geo-alt-fill');
        locationSvg.setAttribute('viewBox', '0 0 16 16');
        
        const locationPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        locationPath.setAttribute('d', 'M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-1a3 3 0 1 1 0-6 3 3 0 0 1 0 6z');
        
        locationSvg.appendChild(locationPath);
        
        const locationSpan = document.createElement('span');
        locationSpan.id = 'popup-location';
        locationSpan.textContent = event.location;
        
        locationDiv.appendChild(locationSvg);
        ...
        buttonsDiv.appendChild(addToCalendarBtn);
        
        eventCard.appendChild(titleH3);
        eventCard.appendChild(dateP);
        eventCard.appendChild(tagsDiv);
        eventCard.appendChild(descP);
    ...
    function createEventCard(event) {
        ...
        letterDiv.className = `letter-card ${event.type.toLowerCase()}`;
        letterDiv.textContent = event.type.charAt(0);
        ...
        const dateP = document.createElement('p');
        dateP.className = 'event-card-date';
        dateP.textContent = formatEventDateTime(event.date, event.time);
        ...
        locationP.className = 'event-card-location';
        ...
        locationSvg.appendChild(locationPath);
        locationP.appendChild(locationSvg);
...
function updateSidebarAuth(user) {
    ...
    if (user) {
        ...
        if (authButtons) {
            ...
            logoutButton.className = 'bg-red-600 text-white px-4 py-2 rounded-full hover:bg-red-700 transition duration-300';
        ...
        if (logoutBtn) {
            ...
            const newLogoutButton = logoutBtn.cloneNode(true);
            logoutBtn.parentNode.replaceChild(newLogoutButton, logoutBtn);
        ...
        if (authButtons) {
            ...
            signInLink.className = 'bg-blue-600 text-white px-4 py-2 rounded-full hover:bg-blue-700 transition duration-300';
