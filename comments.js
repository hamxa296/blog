...
async function toggleReaction(postId, reactionType) {
    ...
    try {
        ...
        if (!existingReaction.empty) {
            ...
            // Update post author's reaction stats
            try {
                const postDoc = await db.collection("posts").doc(postId).get();
                if (postDoc.exists) {
                    const postAuthorId = postDoc.data().authorId;
                    const authorStats = await calculateUserStats(postAuthorId);
                    if (authorStats.success) {
                        await updateUserStats(postAuthorId, authorStats.stats);
                    }
                }
            } catch (error) {
                console.log("Note: Author stats update failed");
            }
...
async function displayReactions(postId) {
    ...
    if (result.success) {
        // Update counts
        document.getElementById('like-count').textContent = result.reactions.like;
        document.getElementById('heart-count').textContent = result.reactions.heart;
        document.getElementById('celebrate-count').textContent = result.reactions.celebrate;
        document.getElementById('insightful-count').textContent = result.reactions.insightful;

        // Update active states
        const user = auth.currentUser;
        if (user) {
            document.getElementById('like-btn').classList.toggle('active', result.userReactions.has('like'));
            document.getElementById('heart-btn').classList.toggle('active', result.userReactions.has('heart'));
            document.getElementById('celebrate-btn').classList.toggle('active', result.userReactions.has('celebrate'));
            document.getElementById('insightful-btn').classList.toggle('active', result.userReactions.has('insightful'));
        }
    }
}
...
async function handleReactionClick(reactionType) {
    ...
    if (result.success) {
        ...
        await displayReactions(currentPostId);
    } else {
        alert(result.error);
    }
}
...
function initializeCommentsAndReactions(postId) {
    ...
    document.getElementById('like-btn').addEventListener('click', () => handleReactionClick('like'));
    document.getElementById('heart-btn').addEventListener('click', () => handleReactionClick('heart'));
    document.getElementById('celebrate-btn').addEventListener('click', () => handleReactionClick('celebrate'));
    document.getElementById('insightful-btn').addEventListener('click', () => handleReactionClick('insightful'));
